...
final class PageRemoveCommand extends FoundationCommand
{
    protected $signature = 'page:remove {name} {--dry-run} {--force}';

    protected $description = 'Remove a page manifest; Blade source is kept unless --force is used.';

    public function handle(FilePlan $p, ProjectRegistry $r): int
    {
        foreach ($r->pages() as $page) {
            if ($page['name'] === $this->argument('name')) {
                $p->delete(str_replace(base_path().'/', '', $page['_file']));
