...
{
    protected $signature = 'module:make:policy {module} {name} {--dry-run} {--force}';

    protected $description = 'Create a policy inside a module.';

    public function handle(FilePlan $p, ModuleArtifactGenerator $g): int
    {
        try {
            $g->plan($p, $this->argument('module'), 'policy', $this->argument('name'));
        } catch (\Throwable $e) {
