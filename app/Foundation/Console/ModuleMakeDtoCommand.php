...
{
    protected $signature = 'module:make:dto {module} {name} {--dry-run} {--force}';

    protected $description = 'Create a dto inside a module.';

    public function handle(FilePlan $p, ModuleArtifactGenerator $g): int
    {
        try {
            $g->plan($p, $this->argument('module'), 'dto', $this->argument('name'));
        } catch (\Throwable $e) {
...
return $this->runPlan($p, 'module:make:dto');
    }
}
