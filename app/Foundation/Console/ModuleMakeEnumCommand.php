...
{
    protected $signature = 'module:make:enum {module} {name} {--dry-run} {--force}';

    protected $description = 'Create a enum inside a module.';

    public function handle(FilePlan $p, ModuleArtifactGenerator $g): int
    {
        try {
            $g->plan($p, $this->argument('module'), 'enum', $this->argument('name'));
        } catch (\Throwable $e) {
...
return $this->runPlan($p, 'module:make:enum');
    }
}
