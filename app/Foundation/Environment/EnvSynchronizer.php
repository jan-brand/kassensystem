...
final class EnvSynchronizer
{
    ...
    public function synchronize(string $templateContent, string $targetContent, bool $force = false, bool $prune = false): array
    {
        ...
        foreach (preg_split('/\R/', rtrim($templateContent, "\r\n")) ?: [] as $line) {
            if (preg_match('/^([A-Z][A-Z0-9_]*)=(.*)$/', $line, $m)) {
                $key = $m[1];
                $seen[$key] = true;
                $value = $force ? $templateValues[$key] : ($targetValues[$key] ?? $templateValues[$key]);
                $lines[] = $key.'='.$value;
            } else {
                $lines[] = $line;
            }
        ...
        if (! $prune && $extras !== []) {
            ...
            foreach ($extras as $key => $value) { $lines[] = $key.'='.$value; }
        }
